...
module.exports = {
  ...
  processMessage: function(_message, callback) {
    ...
    getUser(user).then(function(dbUser) {
      ...
      getCategories(input).then(function(data) {
        var categories = data[0];
        var keywords = data[1];
        debug('2. input.text: %s, extracted categories: %s.', input.text, JSON.stringify(categories));
        debug('2. found keywords %s', JSON.stringify(keywords));
        if (categories) {
      ...
      .then(function(messageResponse) {
        ...
        if(messageResponse.entities) {
            var newKeywords = new Object();

            messageResponse.entities.forEach(function(entity) {
              newKeywords[entity.entity] = entity.value;
            });

            debug('newKeywords object: '+ JSON.stringify(newKeywords));


            if (!messageResponse.context.keywords) {
                messageResponse.context.keywords = newKeywords;
            } else {
                messageResponse.context.keywords = extend(messageResponse.context.keywords, newKeywords);
            }
        }

        return search(messageResponse.context)
        .then(function(products) {
          debug('6. Got narrowed products %s', JSON.stringify(products));
          messageResponse.context.products = products;
      ...
      .then(function(messageToUser) {
        ...
        //debug('Context: %s', JSON.stringify(messageToUser.context));

        return saveUser(dbUser)
        .then(function(data) {
          //debug('Context again: %s', JSON.stringify(messageToUser.context));
