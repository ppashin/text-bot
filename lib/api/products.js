...
module.exports = {
  ...
  search: function(context, callback) {
      ...
      if (context.keywords) {
          ...
          keywords.forEach(function (entity) {
              if (entity.entity == 'color')
                  should.push({ match: { "variation.color": entity.value }});
              else if(entity.entity == 'brand')
                  should.push({ match: { "productBrand.brand": entity.value }});
